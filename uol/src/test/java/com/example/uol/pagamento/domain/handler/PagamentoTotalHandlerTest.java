...
class PagamentoTotalHandlerTest {
    ...
    @Mock
    private PagamentoParcialHandler pagamentoParcialHandler;
    ...
    void testHandle_PagamentoExcedente() {
        ...
        assertEquals(StatusPagamento.TOTAL.toString(), result.getStatus());
        verify(cobrancaRepository).save(cobranca);
        verify(sqsService).sendMessage(result, SqsQueueConstants.PAGAMENTOS_TOTAIS_QUEUE);
    }
